...
class ext4:
	def __init__(self, part):
		...
		##MY VARS##
		self.desc_blocks=int(self.s_blocks_count,16) / int(self.s_blocks_per_group, 16)
		self.desc_blocks_with_super = set([0,1]+[3**x for x in range(1,self.desc_blocks/5) if 3**x<self.desc_blocks] +\
			[5**x for x in range(1,self.desc_blocks/5) if 5**x<self.desc_blocks] + \
			[7**x for x in range(1,self.desc_blocks/5) if 7**x<self.desc_blocks])
